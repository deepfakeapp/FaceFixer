...
def render() -> None:
	...
	BENCHMARK_RESOLUTIONS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = facefixer.choices.benchmark_resolutions,
		value = facefixer.choices.benchmark_resolutions
	)
	BENCHMARK_CYCLE_COUNT_SLIDER = gradio.Slider(
		label = wording.get('uis.benchmark_cycle_count_slider'),
		value = 5,
		step = 1,
		minimum = min(facefixer.choices.benchmark_cycle_count_range),
		maximum = max(facefixer.choices.benchmark_cycle_count_range)
	)
