...
from facefixer import face_landmarker, state_manager, wording
from facefixer.common_helper import calc_float_step
from facefixer.types import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = facefixer.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calc_float_step(facefixer.choices.face_landmarker_score_range),
		minimum = facefixer.choices.face_landmarker_score_range[0],
		maximum = facefixer.choices.face_landmarker_score_range[-1]
	)
