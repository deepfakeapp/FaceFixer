...
from facefixer import core, state_manager
from facefixer.cli_helper import render_table
from facefixer.download import conditional_download, resolve_download_url
from facefixer.filesystem import get_file_extension
from facefixer.types import BenchmarkCycleSet
from facefixer.vision import count_video_frame_total, detect_video_fps, detect_video_resolution, pack_resolution
...
def run() -> Generator[List[BenchmarkCycleSet], None, None]:
	...
	benchmarks = []
	target_paths = [facefixer.choices.benchmark_set.get(benchmark_resolution) for benchmark_resolution in benchmark_resolutions if benchmark_resolution in facefixer.choices.benchmark_set]
