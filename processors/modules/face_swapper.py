...
import facefixer.jobs.job_manager
import facefixer.jobs.job_store
import facefixer.processors.core as processors
from facefixer import config, content_analyser, face_classifier, face_detector, face_landmarker, face_masker, face_recognizer, inference_manager, logger, process_manager, state_manager, video_manager, wording
from facefixer.common_helper import get_first
from facefixer.download import conditional_download_hashes, conditional_download_sources, resolve_download_url
from facefixer.execution import has_execution_provider
from facefixer.face_analyser import get_average_face, get_many_faces, get_one_face
from facefixer.face_helper import paste_back, warp_face_by_face_landmark_5
from facefixer.face_masker import create_area_mask, create_box_mask, create_occlusion_mask, create_region_mask
from facefixer.face_selector import find_similar_faces, sort_and_filter_faces, sort_faces_by_order
from facefixer.face_store import get_reference_faces
from facefixer.filesystem import filter_image_paths, has_image, in_directory, is_image, is_video, resolve_relative_path, same_file_extension
from facefixer.model_helper import get_static_model_initializer
from facefixer.processors import choices as processors_choices
from facefixer.processors.pixel_boost import explode_pixel_boost, implode_pixel_boost
from facefixer.processors.types import FaceSwapperInputs
from facefixer.program_helper import find_argument_group
from facefixer.thread_helper import conditional_thread_semaphore
from facefixer.types import ApplyStateItem, Args, DownloadScope, Embedding, Face, InferencePool, ModelOptions, ModelSet, ProcessMode, QueuePayload, UpdateProgress, VisionFrame
from facefixer.vision import read_image, read_static_image, read_static_images, unpack_resolution, write_image
...
def register_args(program : ArgumentParser) -> None:
	...
	if group_processors:
		...
		facefixer.jobs.job_store.register_step_keys([ 'face_swapper_model', 'face_swapper_pixel_boost' ])
...
def forward_swap_face(source_face : Face, crop_vision_frame : VisionFrame) -> VisionFrame:
	...
	if has_execution_provider('coreml') and model_type in [ 'ghost', 'uniface' ]:
		face_swapper.set_providers([ facefixer.choices.execution_provider_set.get('cpu') ])
